...
require('interpreter.php');				// Includes the interpreter class interpreter.php
...
if (!$conn->connect_error) {			// If connection successful
	...
	if ($sqlrsltGetFeeds = $conn->query($sqlstmtGetFeeds)) {
		...
		$sqlrsltGetFeeds->data_seek(0);	// Pushes the result cursor to the beginning
	...
	die("Could not connect to MYSQL. Fuck<-omg rude. The server is down. Or the login details are wrong. Check /home/notablenews/.secure/secure.ini");
}
